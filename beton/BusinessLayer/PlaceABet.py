...
class PlaceABet:
    ...
    def place_a_bet(self, topic_id, username, option, amount):
        try:
            ...
            Bets.objects.create(topic_id_id = topic_id, username_id = user,option = option, amount = amount)
            bet_info = BetInfo.objects.filter(topic_id_id = topic_id, option = option).values()
            if(len(bet_info) != 1):
            ...
            BetInfo.objects.create(topic_id_id = topic_id, option = option, total_amount = amount, total_users = 1)
